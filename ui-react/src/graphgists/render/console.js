...
export default function(config, ready) {
  var $EDIT_BUTTON, $IFRAME, $IFRAME_WRAPPER, $PLAY_BUTTON, $RESIZE_BUTTON, $RESIZE_VERTICAL_BUTTON, $TOGGLE_CONSOLE_HIDE_BUTTON, $console_template, $resizeOverlay, RESIZE_IN_ICON, RESIZE_OUT_ICON, addConsole, addPlayButtons, consoleClass, consoleUrl, contentId, contentMoveSelector, createConsole, getQueryFromButton, getUrl, gist_id, neo4j_version;
  ...
  contentMoveSelector = 'contentMoveSelector' in config ? config.contentMoveSelector : 'div.navbar';
  ...
  addConsole = function($context, gistId, ready) {
    var $contentMoveSelector, $gistForm, $iframe, $iframeWrapper, $resizeButton, $resizeIcon, $toggleConsoleShowButton, $verticalResizeButton, url;
    ...
    $verticalResizeButton = $RESIZE_VERTICAL_BUTTON.clone().appendTo($iframeWrapper).mousedown(function(event) {
    ...
    $gistForm = $('#gist-form');
    // contextHeight = 0;
    $resizeButton = $RESIZE_BUTTON.clone().appendTo($iframeWrapper).click(function() {
      if ($resizeIcon.hasClass(RESIZE_OUT_ICON)) {
...
const Consolr = function(gistId, neo4j_version) {
  var authenticity_token, currently_querying, establishSession, graph_gist_portal_url, init, process_query_queue, query, query_queue, sessionId;
  sessionId = void 0;
  query_queue = [];
  ...
  process_query_queue = function(final_success, always) {
    ...
    ref = query_queue.shift(), cypher = ref.cypher, success = ref.success, error = ref.error;
    return $.ajax((graph_gist_portal_url()) + "/graph_gists/" + gistId + "/query", {
      method: 'POST',
      data: {
