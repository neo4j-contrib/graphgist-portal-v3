...
export default function($, $content) {
  var COPY_COM_PUBLIC_LINK, DROPBOX_PRIVATE_API_BASE_URL, DROPBOX_PRIVATE_BASE_URL, DROPBOX_PUBLIC_BASE_URL, RISEUP_BASE_URL, VALID_GIST, errorMessage, fetchAnyUrl, fetchDropboxFile, fetchGithubFile, fetchGithubGist, fetchLocalSnippet, fetchPrivateDropboxFile, fetchPublicDropboxFile, getGistAndRenderPage, gist_uuid, neo4jGistFetcher, preview_gist_from_url, readSourceId, useGithubGist, useGithubRepoParts, useRestOfTheUrl;
  ...
  preview_gist_from_url = function(url) {
    window.location.href = '/#!/gists/' + encodeURIComponent(encodeURIComponent(gist_uuid($.trim(url)))) + '?original_url=' + url;
  };
  gist_uuid = function(gist_string) {
    var baseUrl, baseUrls, internal, j, parts, result, sourceParser, sourceParserName, split;
    internal = {};
    internal['sourceParsers'] = {
      ...
      'Etherpad': {
        baseUrl: ['https://beta.etherpad.org/', 'https://piratepad.ca/p/', 'https://factor.cc/pad/p/', 'https://pad.systemli.org/p/', 'https://pad.fnordig.de/p/', 'https://notes.typo3.org/p/', 'https://pad.lqdn.fr/p/', 'https://pad.okfn.org/p/', 'https://beta.publishwith.me/p/', 'https://tihlde.org/etherpad/p/', 'https://tihlde.org/pad/p/', 'https://etherpad.wikimedia.org/p/', 'https://etherpad.fr/p/', 'https://piratenpad.de/p/', 'https://bitpad.co.nz/p/', 'http://beta.etherpad.org/', 'http://notas.dados.gov.br/p/', 'http://free.primarypad.com/p/', 'http://board.net/p/', 'https://pad.odoo.com/p/', 'http://pad.planka.nu/p/', 'http://qikpad.co.uk/p/', 'http://pad.tn/p/', 'http://lite4.framapad.org/p/', 'http://pad.hdc.pw/p/'],
        parse: function(gist, parts, baseUrl) {
