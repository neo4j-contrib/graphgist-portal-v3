...
const GraphGist = function(options, graphgist_cached_queries) {
  var $I, $QUERY_ERROR_LABEL, $QUERY_MESSAGE, $QUERY_OK_LABEL, $QUERY_TOGGLE_BUTTON, $RESULT_TOGGLE_BUTTON, $TABLE_CONTAINER, $TOGGLE_BUTTON, $VISUALIZATION, $VISUALIZATION_ICONS, $WRAPPER, $console_template, $content, $gistId, COLLAPSE_ICON, EXPAND_ICON, HAS_ERRORS, VISUALIZATION_HEIGHT, consolr, content_id, createQueryResultButton, current_display_result_tab_name, display_result_section, executeQueries, findQuery, find_all_next_globally, find_between, find_next_globally, formUrl, gist, handleSelection, initAndGetHeading, most_recent_visulization_number, neod3Renderer, postProcessPage, postProcessRendering, querySearchParams, renderContent, renderGraph, renderTable, replaceNewlines, toggler;
  HAS_ERRORS = false;
  ...
  renderContent = function() {
    ...
    consoleUrl = CONSOLE_VERSIONS[version in CONSOLE_VERSIONS ? version : DEFAULT_VERSION];
    if (querySearchParams()['use_test_console_server'] === 'true') {
      consoleUrl = 'http://neo4j-console-test.herokuapp.com/';
    }
    if (typeof graphgist_cached_queries !== "undefined" && graphgist_cached_queries !== null) {
      ...
      return CypherConsole({
        ...
        if (typeof conslr !== 'undefined') {
          consolr = conslr;
          return typeof consolr.establishSession === "function" ? consolr.establishSession().done(function() {
            return executeQueries((function() {}), postProcessRendering);
          }) : void 0;
        }
      });
  ...
  formUrl = function(url, title, author, twitter) {
    return 'https://docs.google.com/forms/d/1blgZoRZ6vLbpnqdJx3b5c4BkO_mgmD-hgdRQTMm7kc4/viewform?entry.718349727=' + encodeURIComponent(url) + '&entry.1981612324=' + encodeURIComponent(title.length > 18 ? title.substr(0, title.length - 18) : title) + '&entry.1328778537=' + encodeURIComponent(author) + '&entry.507462214=' + encodeURIComponent(twitter);
  };
  ...
  postProcessPage = function() {
    var $footer, $meta, author, authorHtml, number, regex, tags, twitter, version;
    ...
    if (author) {
      authorHtml = '<i class=' + (twitter ? '"icon-twitter-sign"' : '"icon-user"') + '></i> Author ';
      if (twitter) {
    ...
    $('h2[id]').css({
      ...
      return window.location.href = window.location.href.replace(/($|#.+?$)/, '#' + $(this).attr('id'));
    });
  ...
  find_next_globally = function(element, selector) {
    var $current_element, $matching_cousins, $matching_siblings, current_element, i, len, ref, sibling;
  ...
  executeQueries = function(final_success, always) {
    ...
    $elements.each(function(index, element) {
      ...
      success = function(data) {
        var i, j, len, len1, next_query_wrapper, results, showOutput, table_element, table_elements, visualization_element, visualization_elements;
        ...
        table_elements = next_query_wrapper != null ? find_between($element, next_query_wrapper, '.result-table') : find_all_next_globally($element, '.result-table');
        for (i = 0, len = table_elements.length; i < len; i++) {
          table_element = table_elements[i];
          renderTable(table_element, data);
        }
        visualization_elements = next_query_wrapper != null ? find_between($element, next_query_wrapper, '.graph-visualization') : find_all_next_globally($element, '.graph-visualization');
        results = [];
      ...
      error = function(data) {
        HAS_ERRORS = true;
        return createQueryResultButton($QUERY_ERROR_LABEL, $element, data.error, false);
      };
  ...
  renderGraph = function(visualization_element, data, replace) {
    var $visContainer, $visualizationIcons, $visualization_element, actionData, actions, contract, expand, fullscreenClick, iconName, id, keyHandler, rendererHooks, selectedVisualization, show_result_only, sizeChange, style, subscriptions;
    if (replace == null) {
      replace = true;
    }
    ...
    show_result_only = $visualization_element.attr('graph-mode') && $visualization_element.attr('graph-mode').indexOf('result') !== -1;
    ...
    expand = function() {
      ...
      return typeof subscriptions.expand === "function" ? subscriptions.expand() : void 0;
    };
    contract = function() {
      $visContainer.removeClass('fullscreen');
      $visContainer.height(400);
      return typeof subscriptions.contract === "function" ? subscriptions.contract() : void 0;
    };
    sizeChange = function() {
      return typeof subscriptions.sizeChange === "function" ? subscriptions.sizeChange() : void 0;
    };
    ...
    if (data) {
      ...
      subscriptions = 'subscriptions' in rendererHooks ? rendererHooks['subscriptions'] : {};
  ...
  handleSelection = function(data, show_result_only) {
    ...
    hasSelectedRels = data.links.filter(function(link) {
  ...
  renderTable = function(table_element, data, replace, options) {
    ...
    if (!cypherRenderTable($table_container, data, options)) {
      return $table_container.text("Couldn't render the result table.").addClass('alert-error');
    }
  };
  ...
  toggler = function($target, button, action) {
    ...
    if (action && action === 'hide' || action === void 0 && stateIsExpanded) {
      $target.hide();
  ...
  findQuery = function(selector, context, operation) {
    $(selector, context).each(function() {
      $(this).nextAll('div.listingblock').children('div').children('pre.highlight').children('code.language-cypher').first().each(function() {
        operation(this);
      });
    });
  };
